...
| LLM | LangChain.js + Anthropic Claude / OpenAI | Scene gen, video scripting, camera optimization |
| Image Generation | Google Imagen 3 API (primary) | High-quality AI image generation |
| Image Generation Alt | Playwright → Leonardo.ai | Browser-automated fallback |
| Video Generation | Google Veo 2 API (primary) | AI video from image + prompt |
| Video Generation Alt | Playwright → Grok (x.ai) | Browser-automated fallback |
...
export interface VideoData {
  index: number;
  inputImages: string;        // e.g. "IMAGE 2"
  rawPrompt: string;          // from Agent 2
  optimizedPrompt?: string;   // enriched by Agent 3
  generatedUrl?: string;
  localPath?: string;
  status: 'pending' | 'generating' | 'done' | 'error';
}
...
export interface SceneData {
  ...
  characterDescriptions: Record<string, string>; // filtered per scene
...
export const PipelineStateAnnotation = Annotation.Root({
  ...
  messages: Annotation<BaseMessage[], typeof messagesStateReducer>,
});
```
...
```typescript
// src/pipeline/graph/pipeline.graph.ts

const graph = new StateGraph(PipelineStateAnnotation)
  .addNode('generate_scenes',       generateScenesNode)
  .addNode('human_approve_plan',    humanApprovePlanNode)    // ← interrupt
  .addNode('process_scene',         processSceneNode)
  .addNode('human_approve_scene',   humanApproveSceneNode)   // ← interrupt
...
|---|---|---|---|
| `generate_scenes` | `movieIdea` | `scenes[]`, `characterDefinitions`, `storySnapshot` | Fills `char_scene_gen.txt`, parses full story plan |
| `human_approve_plan` | — | — | Interrupt; resumes when user approves or regenerates |
| `process_scene` | `scenes[idx]` | updated `scenes[idx].imageSequence`, `.videoMotionPrompts` | Fills `video_gen.txt` with scene-filtered characters |
| `human_approve_scene` | — | — | Interrupt; resumes when user approves scene script |
| `optimize_camera` | `scenes[idx].videoMotionPrompts` | updated `.optimizedPrompt` per video | Enriches each video prompt with camera move from reference |
| `next_scene_or_finish` | `currentSceneIndex` | incremented index | Advances loop or routes to END |

**Character filtering (per scene):**
Agent 2 receives only the characters listed under "Characters present" in each scene block. The `processSceneNode` extracts these names and filters `characterDefinitions` accordingly before injecting into `video_gen.txt`. This keeps prompts focused and avoids hallucinating absent characters.
...
|---|---|---|---|
| `POST` | `/api/pipeline/start` | `{ movieIdea }` | Start new pipeline, returns `sessionId` |
| `POST` | `/api/pipeline/:id/approve-plan` | `{ regenerate?: boolean }` | Approve or regenerate story plan |
| `POST` | `/api/pipeline/:id/approve-scene` | `{ regenerate?, skip? }` | Approve, regenerate, or skip current scene |
| `GET` | `/api/pipeline/:id/state` | — | Get full current pipeline state |
| `POST` | `/api/pipeline/:id/generate-image` | `{ sceneIndex, imageIndex, prompt }` | Queue image generation job |
| `POST` | `/api/pipeline/:id/generate-video` | `{ sceneIndex, videoIndex, prompt, imagePath }` | Queue video generation job |
| `GET` | `/api/pipeline/:id/download-all` | — | Get all output file paths |
| `GET` | `/output/images/:filename` | — | Serve generated image (static) |
| `GET` | `/output/videos/:filename` | — | Serve generated video (static) |

---

### WebSocket Events

Connect to `ws://localhost:3001/pipeline` and join a session room.
...
| Event | Payload | When |
|---|---|---|
| `pipeline:status` | `{ status: 'running' \| 'idle' }` | Pipeline starts/stops |
| `pipeline:update` | `{ node, state, nextNodes }` | After each node completes |
| `pipeline:interrupt` | `{ type: 'approve_plan' \| 'approve_scene', sceneIndex?, state }` | Human checkpoint reached |
| `pipeline:complete` | `{ state }` | All scenes done |
| `pipeline:error` | `{ message }` | Unhandled error |
| `image:progress` | `{ sceneIndex, imageIndex, status, url? }` | Image job status update |
| `video:progress` | `{ sceneIndex, videoIndex, status, url? }` | Video job status update |

---

### Image Generation

Two providers are supported, configurable via `IMAGE_GEN_PROVIDER` env var:

**Google Imagen 3 (recommended)**
```typescript
// generation/imagen.service.ts
// Uses Google Cloud Vertex AI Prediction API
...
**Leonardo.ai via Playwright (fallback)**
```typescript
// generation/grok.service.ts
// Opens chromium, loads saved auth state, fills prompt, downloads result
// Selector maintenance required when Leonardo UI updates
```
...
```typescript
// generation/veo.service.ts
// Uses Vertex AI video generation endpoint
// Uploads reference image, sends optimized motion prompt
// Polls for completion, downloads MP4
```

**Grok via Playwright (fallback)**
```typescript
// generation/grok.service.ts
// Playwright automation: upload image → enter prompt → wait → download video
// auth/grok_state.json stores session cookies from one-time login
```
...
interface PipelineStore {
  ...
  screen: 'idea' | 'plan' | 'workshop' | 'gallery';

  // Pipeline state (mirrored from backend)
  ...
  // UI state
  isLoading: boolean;
  streamingText: string;     // LLM output streaming in
  interruptType: string | null;

  // Actions
  setSessionId: (id: string) => void;
  setScreen: (screen: Screen) => void;
  updateFromBackend: (state: Partial<PipelineState>) => void;
  updateImageStatus: (sceneIdx: number, imgIdx: number, status: ImageData) => void;
  updateVideoStatus: (sceneIdx: number, vidIdx: number, status: VideoData) => void;
}
```
...
export function useSocket(sessionId: string | null) {
  ...
  useEffect(() => {
    ...
    socket.on('pipeline:interrupt', ({ type, state }) => {
    ...
    socket.on('pipeline:complete', ({ state }) => {
      store.updateFromBackend(state);
      store.setScreen('gallery');
    });

    socket.on('image:progress', ({ sceneIndex, imageIndex, status, url }) => {
      store.updateImageStatus(sceneIndex, imageIndex, { status, generatedUrl: url });
    });

    socket.on('video:progress', ({ sceneIndex, videoIndex, status, url }) => {
      store.updateVideoStatus(sceneIndex, videoIndex, { status, generatedUrl: url });
    });
